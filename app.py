...
def fetch_clean_data():
    # Load file and clean invisible characters
    df = pd.read_csv('Refined_Employee_Database.csv')
    df = df.loc[:, ~df.columns.str.contains('^Unnamed')]
    df.columns = df.columns.str.strip()
    
    # Dynamically find columns to prevent KeyError
    ...
    if salary_col: df.rename(columns={salary_col: 'Annual Salary'}, inplace=True)
    if bonus_col: df.rename(columns={bonus_col: 'Bonus %'}, inplace=True)
    if exit_col: df.rename(columns={exit_col: 'is_exited'}, inplace=True)
...
with col1:
    ...
    # The mathematical attrition calculation
    if 'is_exited' in df.columns:
        df['attrition_numeric'] = df['is_exited'].astype(str).str.upper().eq('YES').astype(int)
...
if 'Age_Group' in df.columns and 'Job_Level' in df.columns:
